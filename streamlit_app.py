...
with tab1:
    try:
        # daily_variance is a view, ensuring we handle log_date correctly
...
with tab3:
    ...
    try:
        # CHANGED: Order by 'log_date' instead of 'date'
        a_res = (
...
with tab4:
    ...
    try:
        # CHANGED: All sort operations updated to log_date
        res = (
            supabase.table(tbl)
            .select("*")
            .order("log_date", desc=True)
    ...
    if st.button("🚀 Prepare Master CSV", key="master_exp_btn"):
        try:
            ...
            for i in h_res.data:
                # Check both just in case view vs table naming differs
                date_val = i.get("log_date") or i.get("date")
                if i.get("weight_lb"):
                    master.append(
                        {
                            "date": date_val,
